...
AVOIDANCE 진입 조건:
  tof_alert OR lidar_alert OR depth_alert  ← 어느 센서 하나라도 감지
...
class ObstacleDetectorNode(Node):
    def __init__(self):
        ...
        self._lidar_side_min = float('inf')
        self._depth_front    = float('inf')  # depth 카메라 정면 최솟값 (m)
        self._latched = False

        self.create_subscription(Float32,   '/tof/front_distance',   self._on_tof,   10)
        self.create_subscription(Float32,   '/depth/front_distance', self._on_depth, 10)
        ...
        self.get_logger().info(
            ...
            f'<{self._side_th}m | depth enabled'
        )
    ...
    def _on_depth(self, msg: Float32):
        self._depth_front = float(msg.data) if math.isfinite(msg.data) else float('inf')
    ...
    def _tick(self):
        ...
        # 3단계: Depth 카메라 보조 확인 (없으면 False → 기존 2단계 로직 유지)
        depth_alert = math.isfinite(self._depth_front) and self._depth_front < self._enter

        # 측면 대각 감지 (즉시 진입, ToF 확인 불필요)
        side_alert = math.isfinite(self._lidar_side_min) and self._lidar_side_min < self._side_th

        # AVOIDANCE 진입: 어느 센서 하나라도 감지하면 즉시 진입
        enter_cond = tof_alert or lidar_alert or depth_alert or side_alert
        ...
        if not self._latched and enter_cond:
            ...
            if side_alert:
                ...
                reason = (f'FRONT(ToF={self._tof_front:.2f} '
                          f'LiDAR={self._lidar_front:.2f} '
                          f'Depth={self._depth_front:.2f})')
        ...
        fused = min(self._tof_front, self._lidar_front, self._lidar_side_min, self._depth_front)
